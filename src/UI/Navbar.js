...
export default function Navbar() {
  ...
  return (
    <nav className="fixed flex items-center inset-x-0 top-0 px-6 py-3 justify-between w-full bg-purple-900">
      <Link to="/">
        <h1 className="text-3xl text-white">Podcasts</h1>
      </Link>
      {isMobile && (
        <button onClick={() => setIsNavbarActive(true)}>
          <img src={menu} alt="menu" />
        </button>
      )}
      {isNavbarActive && (
        <Backdrop clickAction={() => setIsNavbarActive(false)} />
      )}
      <div
        className={`items-center flex 
                    ${
                      isMobile &&
                      `border-box py-4 px-2 bg-gray-700 fixed inset-y-0 right-0 w-10/12 h-screen z-30 flex flex-col -right-full
