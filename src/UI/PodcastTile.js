...
export default function PodcastTile({ data, CRUDMode }) {
  ...
  return (
    <div
      onClick={() => setPreviewedPodcast(data)}
      className="card w-64 mr-2"
      style={{ minWidth: "16rem" }}
    >
      <img
        className="w-full object-cover max-h-56 h-56"
        src={`${env.BASE_URL}api/podcasts/image/${data.cover_img}`}
        alt="Card image cap"
      />
      <div className="p-3">
        <h5 className="text-2xl md:text-4xl mb-2 truncate">{data.title}</h5>
        <p
          onClick={(e) => goToAuthorProfileHandler(e, data.author.id)}
          className="mb-2 text-lg"
        >
          {data.author.username}
        </p>
        <div className="flex justify-between">
          <button onClick={(e) => setGlobalPodcast(data, e)} className="btn">
            {t("button.play")}
          </button>
          {CRUDMode && currentUser && currentUser.id == data.author.id && (
            <>
              <button className="btn" onClick={showEditFormHandler}>
                {t("button.edit")}
              </button>
              <button className="btn-danger" onClick={showDeleteFormHandler}>
                {t("button.delete")}
              </button>
            </>
          )}
      ...
      {isDeleteFormVisible && (
        ...
        >
          <DeleteForm
            cancelHAndler={() => setIsDeleteFormVisible(false)}
            podcast={data}
          />
        </Backdrop>
      )}
      ...
      {isEditFormVisible && (
        ...
        >
          <EditPodcastForm
            cancelHAndler={() => setIsEditFormVisible(false)}
            podcast={data}
          />
        </Backdrop>
      )}
